...
import { listAllMainMarket } from "@/api/functions";
...
const MainMarket = ({ search }: { search: string }) => {
  ...
  useEffect(() => {
    onLoading(loading, setLoading, async () => {
      try {
        let result = await listAllMainMarket();
        setChartData(result);
            ...
            <Link href={`/stock/${chartData.id}`}>
              <div className="flex justify-between items-center w-full py-2 px-5">
                <div className="flex flex-col justify-between items-start">
                  ...
                  <p className="text-xs font-normal font-inter text-black whitespace-nowrap min-w-[75px] lg:min-w-[150px] truncate max-w-[75px] lg:max-w-[150px]">
                    {chartData?.stockName}
                  </p>
                </div>
                ...
                <div className="flex flex-col justify-between items-end">
                  <h2 className="text-base font-semibold font-inter text-black whitespace-nowrap">
